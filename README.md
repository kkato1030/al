...
| `al diff` | 全プロファイルを合成した望ましい状態とシステムの現在の状態を比較して差分を表示。`+` は未インストール、`~` はアップグレード可能、`-` はいずれのプロファイルにも無いパッケージ。差分がある場合は exit code 1 を返す |
...
1. `al diff` で全プロファイルを合成した状態とシステムを比較
2. 差分があれば `al sync` で同期、差分がなければ exit code 0
