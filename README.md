...
1. **使用目的**: work（仕事）/ personal（個人）/ mixed（両方）
2. **プロファイル名**: mixed を選んだ場合のみ入力（work/personal は自動設定）
3. **trial ワークフローの有効化**: 実験的パッケージの試用期間を設けるか
4. **レビュー期間**: trial を有効にした場合、期限日数（7/14/30/60日）を選択

最小限の質問で、使いやすい初期設定が完成。
...
| 差分確認 | 期待する状態（profiles）と現在の状態を比較 | `al diff` |
...
| Brewfile 取り込み | Brewfile または brew/mas の現在インストール状況を al 管理下に登録 | `al import [Brewfile] --prf <profile>` (Brewfile 省略時は自動検出) |
...
| `al package import [Brewfile]` | Brewfile から取り込み、または brew/mas から自動検出してインポート（`--prf`, `--install`, `--dry-run`, `-i` など） |
