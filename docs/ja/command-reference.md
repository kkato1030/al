...
| `al backup` | `~/.al` を commit して GitHub に push（`--init` でリポジトリ作成、`--dry-run` でプレビュー）。`--pull` でリモートの変更を取り込むことも可能（`al pull` を推奨） |
| `al pull` | リモートバックアップの変更を `~/.al` にフェッチ・マージ。`--repo` でリポジトリを上書き指定可、`--dry-run` でプレビュー。コンフリクト時は解決方法を案内 |
